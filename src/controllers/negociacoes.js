...
module.exports = {
  // GET /negociacoes
  async listarNegociacoes(req, res) {
    try {
      const { page, limit, offset } = buildPagination(req.query);
      const { where, values } = pickFilters(req.query, ["neg_status", "neg_observacao"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(neg_ativa AS UNSIGNED) AS neg_ativa
                   FROM negociacoes
                   LEFT JOIN propostas p ON p.pro_neg_id = negociacoes.neg_id

                   ${whereSql}
                   ORDER BY neg_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de negociacoes.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /negociacoes
  async cadastrarNegociacoes(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO negociacoes (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Negociacoes cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async editarNegociacoes(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT neg_id FROM negociacoes WHERE neg_id = ?', [id]);
      ...
      const sql = `UPDATE negociacoes SET ${fields.join(', ')} WHERE neg_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Negociacoes atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async apagarNegociacoes(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT neg_id FROM negociacoes WHERE neg_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM negociacoes WHERE neg_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Negociacoes excluído(a).', { id });
      } catch (e) {
