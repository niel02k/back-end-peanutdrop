...
module.exports = {
  ...
  async listarEmpresas(req, res) {
    try {
      ...
      const { where, values } = pickFilters(req.query, ["emp_razao_social", "emp_nome_fantasia", "emp_tipo_atividade"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(emp_ativo AS UNSIGNED) AS emp_ativo
                   FROM empresas
                   ${whereSql}
                   ORDER BY emp_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de empresas.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /empresas
  async cadastrarEmpresas(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO empresas (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Empresas cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // PATCH /empresas/:id
  async editarEmpresas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT emp_id FROM empresas WHERE emp_id = ?', [id]);
      ...
      const sql = `UPDATE empresas SET ${fields.join(', ')} WHERE emp_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Empresas atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // DELETE /empresas/:id
  async apagarEmpresas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT emp_id FROM empresas WHERE emp_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM empresas WHERE emp_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Empresas excluído(a).', { id });
      } catch (e) {
