...
module.exports = {
  // GET /mensagem
  async listarMensagem(req, res) {
    try {
      const { page, limit, offset } = buildPagination(req.query);
      const { where, values } = pickFilters(req.query, ["men_texto"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(men_ativo AS UNSIGNED) AS men_ativo
                   FROM mensagem
                   ${whereSql}
                   ORDER BY men_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de mensagem.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async cadastrarMensagem(req, res) {
    try {
      ...
      const sql = `INSERT INTO mensagem (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Mensagem cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async editarMensagem(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT men_id FROM mensagem WHERE men_id = ?', [id]);
      ...
      const sql = `UPDATE mensagem SET ${fields.join(', ')} WHERE men_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Mensagem atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // DELETE /mensagem/:id
  async apagarMensagem(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT men_id FROM mensagem WHERE men_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM mensagem WHERE men_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Mensagem excluído(a).', { id });
      } catch (e) {
