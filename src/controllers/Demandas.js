...
module.exports = {
  ...
  async listarDemandas(req, res) {
    try {
      ...
      const { where, values } = pickFilters(req.query, ["dem_titulo", "dem_descricao"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(dem_ativo AS UNSIGNED) AS dem_ativo
                   FROM demandas
                   ${whereSql}
                   ORDER BY dem_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de demandas.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /demandas
  async cadastrarDemandas(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO demandas (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Demandas cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // PATCH /demandas/:id
  async editarDemandas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT dem_id FROM demandas WHERE dem_id = ?', [id]);
      ...
      const sql = `UPDATE demandas SET ${fields.join(', ')} WHERE dem_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Demandas atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // DELETE /demandas/:id
  async apagarDemandas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT dem_id FROM demandas WHERE dem_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM demandas WHERE dem_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Demandas excluído(a).', { id });
      } catch (e) {
