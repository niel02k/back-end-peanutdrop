...
module.exports = {
  // GET /notas_fiscais
  async listarNotas_fiscais(req, res) {
    try {
      const { page, limit, offset } = buildPagination(req.query);
      const { where, values } = pickFilters(req.query, ["nf_numero", "nf_chave_acesso"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(nf_cancelada AS UNSIGNED) AS nf_cancelada
                   FROM notas_fiscais
                   ${whereSql}
                   ORDER BY nf_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de notas_fiscais.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /notas_fiscais
  async cadastrarNotas_fiscais(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO notas_fiscais (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Notas_fiscais cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },

  // PATCH /notas_fiscais/:id
  async editarNotas_fiscais(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT nf_id FROM notas_fiscais WHERE nf_id = ?', [id]);
      ...
      const sql = `UPDATE notas_fiscais SET ${fields.join(', ')} WHERE nf_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Notas_fiscais atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async apagarNotas_fiscais(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT nf_id FROM notas_fiscais WHERE nf_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM notas_fiscais WHERE nf_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Notas_fiscais excluído(a).', { id });
      } catch (e) {
