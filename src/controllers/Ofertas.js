...
module.exports = {
  ...
  async listarOfertas(req, res) {
    try {
      ...
      const { where, values } = pickFilters(req.query, ["ofe_titulo", "ofe_descricao"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(ofe_promocao AS UNSIGNED) AS ofe_promocao, CAST(ofe_ativo AS UNSIGNED) AS ofe_ativo
                   FROM ofertas
                   ${whereSql}
                   ORDER BY ofe_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de ofertas.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /ofertas
  async cadastrarOfertas(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO ofertas (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Ofertas cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // PATCH /ofertas/:id
  async editarOfertas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT ofe_id FROM ofertas WHERE ofe_id = ?', [id]);
      ...
      const sql = `UPDATE ofertas SET ${fields.join(', ')} WHERE ofe_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Ofertas atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // DELETE /ofertas/:id
  async apagarOfertas(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT ofe_id FROM ofertas WHERE ofe_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM ofertas WHERE ofe_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Ofertas excluído(a).', { id });
      } catch (e) {
