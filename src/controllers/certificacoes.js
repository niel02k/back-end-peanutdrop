...
module.exports = {
  // GET /certificacoes
  async listarCertificacoes(req, res) {
    try {
      const { page, limit, offset } = buildPagination(req.query);
      const { where, values } = pickFilters(req.query, ["cer_nome", "cer_orgao"]);
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const sql = `SELECT *, CAST(cer_ativa AS UNSIGNED) AS cer_ativa
                   FROM certificacoes
                   ${whereSql}
                   ORDER BY cer_id DESC
                   LIMIT ? OFFSET ?`;
      const [rows] = await db.query(sql, [...values, limit, offset]);
      sendOk(res, 'Lista de certificacoes.', rows);
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  // POST /certificacoes
  async cadastrarCertificacoes(req, res) {
    try {
      const data = req.body;
      ...
      const sql = `INSERT INTO certificacoes (${cols.join(', ')}) VALUES (${qMarks})`;
      const [result] = await db.query(sql, vals);
      sendCreated(res, 'Certificacoes cadastrado(a).', { id: result.insertId });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async editarCertificacoes(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT cer_id FROM certificacoes WHERE cer_id = ?', [id]);
      ...
      const sql = `UPDATE certificacoes SET ${fields.join(', ')} WHERE cer_id = ?`;
      values.push(id);
      const [result] = await db.query(sql, values);
      sendOk(res, 'Certificacoes atualizado(a).', { id, linhas_afetadas: result.affectedRows });
    } catch (error) {
      sendError(res, error);
    }
  },
  ...
  async apagarCertificacoes(req, res) {
    try {
      const { id } = req.params;
      const [check] = await db.query('SELECT cer_id FROM certificacoes WHERE cer_id = ?', [id]);
      if (!check.length) return sendNotFound(res);
      try {
        const [r] = await db.query('DELETE FROM certificacoes WHERE cer_id = ?', [id]);
        if (!r.affectedRows) return sendNotFound(res);
        sendOk(res, 'Certificacoes excluído(a).', { id });
      } catch (e) {
